...
/**
 * Save parsed questions to Firebase in the background for backward compatibility.
 */
...
export async function POST(req: NextRequest) {
  ...
  // 1. Generate the paper blueprint deterministically using PYQs
  let blueprint;
  try {
    ...
    return NextResponse.json<GenerateError>(
      { error: "Failed to generate paper blueprint", code: "UNKNOWN" },
      { status: 500 }
    );
  }
  ...
  // Generate a paperId for linking
  const paperId = `paper_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;

  // Slice blueprint into chunks (e.g., 5 questions per chunk)
  const CHUNK_SIZE = 5;
  ...
  // Map questions and inject database IDs
  ...
  // Create paper document using blueprint stats, but fallback to runtime computation if not present
  const paperDoc = {
    paperId,
    userId: uid,
    subject,
  ...
  // Save paper document and individual questions in parallel
  if (idToken && uid) {
    await savePaperToFirestore(idToken, paperDoc);
    // Save individual questions asynchronously for backward-compatibility
